...
class Tetris(object):
	...
	def start_game(self) -> None:
		if self.gameover:
			...
			self.new_stone()
			self.lines = 0
			self.timer_delay = self._config['delay']
			self.gameover = False
	...
	def new_stone(self) -> None:
		...
		if self.check_collision(self.board, self.stone, (self.stone_x, self.stone_y)):
			print(f"Game over - lines: {self.lines}")
			self.gameover = True
		...
		if not self.gameover and not self.paused:
			...
			if self.check_collision(self.board, self.stone, (self.stone_x, self.stone_y)):
				...
				if removed_lines:
					...
					new_delay = int(self._config['delay'] * (0.9 ** (self.lines // 5)))
					if new_delay < 1:
						new_delay = 1
					if new_delay != self.timer_delay:
						self.timer_delay = new_delay
						pygame.time.set_timer(pygame.USEREVENT+1, self.timer_delay)
